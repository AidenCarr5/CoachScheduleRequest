...
function withoutTournamentConflicts(data) {
  ...
  return {
    ...data,
    conflictEvents: data.conflictEvents.filter((event) => !eventIsTournament(event) || eventIsHostedTournamentAvailability(event))
  };
}
...
async function refreshDateData(date) {
  ...
  const promise = (async () => {
    ...
    const partial = await generateDateData(dateIso);
