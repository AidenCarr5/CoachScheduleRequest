...
function createSeasonPlanner({ rootDir, storageDir, teamLabel }) {
  ...
  function addAdminAccount(account) {
    ...
    const next = {
      ...
      canAccessTitans: account.canAccessTitans !== false,
      canAccessAthletics: account.canAccessAthletics !== false,
  ...
  function saveAdminPrivilegeOverrides(privileges) {
    ...
    store.adminPrivileges = (Array.isArray(privileges) ? privileges : [])
      .map((item) => ({
        ...
        canAccessTitans: item.canAccessTitans === true,
        canAccessAthletics: item.canAccessAthletics === true,
