...
function createSeasonPlanner({ rootDir, storageDir, teamLabel }) {
  ...
  function seasonId(year) {
    return `season-${String(year || new Date().getFullYear()).replace(/[^0-9]/g, '') || Date.now()}`;
  }
  ...
  function ensureSeason(year, label = '') {
    ...
    if (!season) {
      season = {
        ...
        label: label || `${Number(year) || new Date().getFullYear()} Season`,
        status: 'setup',
        coaches: [],
  ...
  function normalizeUploadedEvent(raw, coach) {
    const date = normalizeDate(raw.date || raw.Date || raw.day || raw.Day);
    const start = formatTime(raw.start || raw.Start || raw.time || raw.Time);
    const end = formatTime(raw.end || raw.End || raw.finish || raw.Finish);
    const type = normalizeType(raw.type || raw.Type || raw.eventType || raw['Event Type']);
    const diamond = String(raw.diamond || raw.Diamond || raw.venue || raw.Venue || raw.location || raw.Location || '').trim();
    const opponent = String(raw.opponent || raw.Opponent || raw.title || raw.Title || raw.description || raw.Description || '').trim();
