...
async function loginSchedulingShell(page) {
  ...
  await page.goto(scheduleShellUrl(), { waitUntil: 'domcontentloaded' });
...
async function confirmCancellationFromModal(page, request, replacementType = '') {
  ...
  const cancellationMode = await deleteFrame.evaluate(({ originalType, replacementType }) => {
    ...
    replacementType
  }).catch(() => (replacementType ? 'Cancel' : 'Single'));

  await deleteFrame.evaluate(({ reason, replacementType }) => {
    const radioName = 'ctl00$cM$rblMulti';
    const radios = [...document.querySelectorAll(`input[name="${radioName}"]`)];
    radios.forEach((radio) => {
      radio.checked = radio.value === replacementType;
    });
    ...
    if (replaceWithPractice) {
      replaceWithPractice.checked = String(replacementType || '') === 'Practice';
    }
  }, {
    reason: request.reason || '',
    replacementType: cancellationMode
  }).catch(() => {});
...
async function cancelGame(jar, request, replacementType = '') {
  ...
  form.values.__EVENTTARGET = 'ctl00$cC$btnDelete';
  form.values.__EVENTARGUMENT = '';
  form.values['ctl00$cM$rblMulti'] = chooseCancellationMode(radioGroupValuesFromHtml(html, 'ctl00$cM$rblMulti'), request, replacementType);
  form.values['ctl00$cM$txtReason'] = request.reason || '';
...
async function applyApprovedRequest(request) {
  ...
  if ((request.action || '').startsWith('Replace ')) {
    ...
    if (/practice/i.test(request.newType)) {
      await createPractice(jar, request);
      return;
    }
    await createGame(jar, request);
    return;
  }
