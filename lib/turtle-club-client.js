...
function cpTournamentDivisionUrl(tournamentId = 3331, divisionId = '') {
  return `${baseUrl}/CP/#Module=Tournament;ParentID=${encodeURIComponent(tournamentId)};SelectedValue=/CP/Live/Tournaments/${encodeURIComponent(tournamentId)}/Divisions/${encodeURIComponent(divisionId)}/`;
}
...
async function tryOpenModalFromDayAtGlance(page, mainFrame, request, kind) {
  ...
  if (kind === 'practice' || kind === 'local') {
    const menuLabel = kind === 'local' ? 'Local Game' : 'Practice';
    const practiceCandidates = [
      `text=${menuLabel}`,
      `a:has-text("${menuLabel}")`,
      `button:has-text("${menuLabel}")`
    ];
...
function requestIsLocalGame(request) {
  return /\blocal\s+game\b/i.test(`${request && request.newType || ''} ${request && request.action || ''}`);
}

async function setLocalGameMode(frame) {
  await frame.evaluate(() => {
    const hidden = document.getElementById('cM_hfIsInternal')
      || document.querySelector('input[name="ctl00$cM$hfIsInternal"]');
  ...
  try {
    ...
    try {
      ...
      modalFrame = await openShellModal(page, mainFrame, async (frame) => {
        ...
        await frame.evaluate((isLocal) => ShowInsertGameForm(isLocal ? 1 : 0), kind === 'local');
      });
    }
    ...
    if (kind === 'practice') {
      ...
      if (kind === 'local') {
        await setLocalGameMode(modalFrame);
      } else {
...
function parseCpEventBlock(block, date) {
  ...
  if (/pnlLocal/i.test(className) || /\blocal\s+game\b/i.test(`${subject} ${opponent}`)) eventKind = 'Local Game';
  ...
  return {
    ...
    team: organization === organizationName ? rawTeam : `${organization} - ${rawTeam}`,
    diamond: venue,
...
function eventKindMatches(request, event) {
  ...
  if (originalType.includes('home')) return eventKind.includes('home') || eventKind.includes('local');
  if (originalType.includes('local') && originalType.includes('game')) return eventKind.includes('local') || eventKind.includes('home') || eventKind.includes('game');
...
function eventMatchesRequest(request, event) {
  ...
  if (requestOpponent && requestOpponent !== 'practice') {
    ...
    if (requestOrEventIsLocalGame(request, event) && (!eventOpponent || !requestOpponent)) {
      return true;
    }
...
function eventMatchesNewRequest(request, event) {
  ...
  if (requestedType.includes('home') && !(eventKind.includes('home') || eventKind.includes('local'))) return false;
  if (requestedType.includes('local') && requestedType.includes('game') && !(eventKind.includes('local') || eventKind.includes('home') || eventKind.includes('game'))) return false;
...
async function clickInsertAndCloseFromHtml(jar, html, request, kind) {
  ...
  if (kind === 'practice') {
    ...
    if (kind === 'local' || requestIsLocalGame(request)) {
      form.values['ctl00$cM$hfIsInternal'] = 'True';
    } else {
...
async function createGame(jar, request) {
  assertNotPastCreate(request);
  const beforeEvents = await collectMatchingEvents(jar, request);
  const kind = requestIsLocalGame(request) ? 'local' : 'game';
  const { browser, page, preparedHtml } = await createViaShell(request, kind);
  try {
    let { created } = await waitForCreatedEvent(jar, request, beforeEvents, { timeoutMs: 20000, intervalMs: 2000 });
    if (!created) {
      ({ created } = await waitForCreatedEventOnPage(page, request, beforeEvents, { timeoutMs: 12000, intervalMs: 2000 }));
    ...
    if (modalFrame && await modalFrame.locator('#ctl00_cC_rwConflicts_C_btn_ignore').isVisible().catch(() => false)) {
      ...
      if (preparedHtml) {
        const retryResult = await clickInsertAndCloseFromHtml(jar, preparedHtml, request, kind);
...
function inningsToDefensiveOuts(value) {
  const raw = String(value ?? '').trim();
  if (!raw) return null;
  const match = raw.match(/^(\d+)(?:[.\s-]?([012]))?$/);
  if (!match) return null;
  const innings = Number(match[1]);
  const partialOuts = match[2] === undefined ? 0 : Number(match[2]);
  if (!Number.isInteger(innings) || innings < 0 || !Number.isInteger(partialOuts) || partialOuts < 0 || partialOuts > 2) return null;
  return (innings * 3) + partialOuts;
}

function defensiveOutsValue(outsValue, inningsValue) {
  if (outsValue !== undefined && outsValue !== null && String(outsValue).trim() !== '') {
    const outs = Number(outsValue);
    return Number.isInteger(outs) && outs >= 0 ? outs : null;
  }
  return inningsToDefensiveOuts(inningsValue);
}
...
async function getTournamentBracketData({ tournamentId = 3331 } = {}) {
  ...
  const divisions = extractLinks(divisionsHtml)
    .map((link) => {
      ...
      return {
        ...
        cpUrl: cpTournamentDivisionUrl(tournamentId, match[1])
      };
    })
    .filter(Boolean)
  ...
  const divisionsWithGames = standings.map((division) => {
    ...
    return {
      ...division,
      games: divisionGames,
      bracketGames: divisionGames.filter((game) => game.isBracket)
    };
  });
...
async function listCpTournamentScoreGames({ tournamentId = 3331, mode = 'unreported', status = 'unreported', requireResult = false } = {}) {
  const browser = await launchAutomationBrowser();
  const page = await browser.newPage({ viewport: { width: 1600, height: 1200 } });
  try {
    await loginTournamentShell(page, tournamentId);
    const frame = await openTournamentScheduleFrame(page, tournamentId, mode);
    saveDebugHtml(status === 'reported' ? 'tournament-reported-score-list.html' : 'tournament-score-list.html', await frame.content().catch(() => ''));
    ...
    return games
      ...
      .filter((row) => !requireResult || String((row.cells || [])[5] || '').trim())
      .map((row) => {
...
async function listTournamentScoreGames({ tournamentId = 3331 } = {}) {
  return listCpTournamentScoreGames({ tournamentId, mode: 'unreported', status: 'unreported' });
}

function displayTournamentDateLabel(dateIso) {
  if (!dateIso) return '';
...
async function listPublicReportedTournamentScoreGames({ tournamentId = 3331 } = {}) {
  const games = await listPublicTournamentSchedule(tournamentId);
  return games
    .filter((game) => String(game.score || '').trim())
    .map((game, index) => ({
      id: game.id,
      tournamentId,
      rowIndex: index,
      date: game.date || '',
      dateLabel: displayTournamentDateLabel(game.date),
      time: game.time || '',
      division: game.age || '',
      ...
      status: 'reported',
      action: { text: 'Score' },
      summary: [displayTournamentDateLabel(game.date), game.age, game.gameNumber, game.time, game.venue, game.visitor, game.home].filter(Boolean).join(' | ')
    }));
}

function tournamentReportedGameMergeKey(game) {
...
async function listReportedTournamentScoreGames({ tournamentId = 3331 } = {}) {
  const publicGames = await listPublicReportedTournamentScoreGames({ tournamentId }).catch(() => []);
  const cpGames = await listCpTournamentScoreGames({ tournamentId, mode: '', status: 'reported', requireResult: true }).catch(() => []);
  const merged = new Map();
  publicGames.forEach((game) => merged.set(tournamentReportedGameMergeKey(game), game));
  cpGames.forEach((game) => {
    const key = tournamentReportedGameMergeKey(game);
    merged.set(key, { ...(merged.get(key) || {}), ...game, status: 'reported' });
  });
  return [...merged.values()]
    .filter((game) => String(game.result || game.score || '').trim())
    .sort((left, right) => `${left.date || ''} ${left.time || ''} ${left.division || ''} ${left.gameNumber || ''}`.localeCompare(`${right.date || ''} ${right.time || ''} ${right.division || ''} ${right.gameNumber || ''}`));
}
...
async function submitTournamentScore({ tournamentId = 3331, game, homeScore, awayScore, visitorDefensiveOuts, homeDefensiveOuts, visitorDefensiveInnings, homeDefensiveInnings }) {
  ...
  const parsedVisitorDefensiveOuts = defensiveOutsValue(visitorDefensiveOuts, visitorDefensiveInnings);
  const parsedHomeDefensiveOuts = defensiveOutsValue(homeDefensiveOuts, homeDefensiveInnings);
  if (!Number.isInteger(parsedVisitorDefensiveOuts) || !Number.isInteger(parsedHomeDefensiveOuts)) {
    throw new Error('Enter defensive innings as whole innings or baseball notation such as 6.1 or 6.2.');
  }
  if (!game || !game.id) {
  ...
  try {
    await loginTournamentShell(page, tournamentId);
    const mode = game && game.status === 'reported' ? '' : 'unreported';
    const frame = await openTournamentScheduleFrame(page, tournamentId, mode);
    ...
    const filled = await editor.evaluate(({ home, visitor, visitorOuts, homeOuts }) => {
      function clean(value) {
        return String(value || '').replace(/\s+/g, ' ').trim();
      }
      ...
      const visitorOutsField = document.getElementById('cM_rtbVDefOuts')
        || inputs.find((item) => /vdef|visitor.*def|away.*def|visitor.*out/i.test(`${item.id} ${item.name} ${item.placeholder} ${item.label}`))?.input;
      const homeOutsField = document.getElementById('cM_rtbHDefOuts')
        || inputs.find((item) => /hdef|home.*def|home.*out/i.test(`${item.id} ${item.name} ${item.placeholder} ${item.label}`))?.input;
      ...
      if (!chosenVisitor || !chosenHome || !visitorOutsField || !homeOutsField) return { ok: false, count: fallback.length, hasVisitorOuts: Boolean(visitorOutsField), hasHomeOuts: Boolean(homeOutsField) };
      ...
      visitorOutsField.value = String(visitorOuts);
      visitorOutsField.dispatchEvent(new Event('input', { bubbles: true }));
      visitorOutsField.dispatchEvent(new Event('change', { bubbles: true }));
      homeOutsField.value = String(homeOuts);
      homeOutsField.dispatchEvent(new Event('input', { bubbles: true }));
      homeOutsField.dispatchEvent(new Event('change', { bubbles: true }));
      return {
        ok: true,
        fields: [
          { role: 'visitor', id: chosenVisitor.id || '', name: chosenVisitor.name || '' },
          { role: 'home', id: chosenHome.id || '', name: chosenHome.name || '' },
          { role: 'visitorDefensiveOuts', id: visitorOutsField.id || '', name: visitorOutsField.name || '', value: visitorOuts },
          { role: 'homeDefensiveOuts', id: homeOutsField.id || '', name: homeOutsField.name || '', value: homeOuts }
        ]
      };
    }, { home: parsedHome, visitor: parsedVisitor, visitorOuts: parsedVisitorDefensiveOuts, homeOuts: parsedHomeDefensiveOuts });
    if (!filled || !filled.ok) {
    ...
    return {
      ...
      visitorDefensiveOuts: parsedVisitorDefensiveOuts,
      homeDefensiveOuts: parsedHomeDefensiveOuts
    };
  } catch (error) {
...
module.exports = {
  ...
  listReportedTournamentScoreGames,
