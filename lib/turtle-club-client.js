...
async function updateGameOpponent(request) {
  ...
  try {
    const response = await fetchWithJar(jar, editUrl);
    const html = await response.text();
    saveDebugHtml('opponent-update-edit-form.html', html);
    if (!response.ok || /Login Page|Forgot Password|Human Verification/i.test(html)) {
      throw new Error('Authenticated Turtle Club game editor request did not return the game edit form.');
    }
    const form = parseDefaultForm(html);
    rehydrateRadDateTimeFields(form.values, html);
    rehydrateComboFields(form.values, html);
    selectCombo(form.values, html, 'ctl00_cM_rcbTeams2', request.opponent, { stateMode: 'selected' });
    form.values.__EVENTTARGET = 'ctl00$cC$lbUpdateAndClose';
    form.values.__EVENTARGUMENT = '';
    delete form.values.__ASYNCPOST;
    delete form.values['ctl00$sm'];
    saveDebugHtml('opponent-update-edit-request.json', JSON.stringify(form.values, null, 2));
    const result = await postForm(jar, buildActionUrl(form.action || editUrl), form.values);
    saveDebugHtml('opponent-update-edit-result.html', result);
    if (/Login Page|Forgot Password|Human Verification/i.test(result)) {
      throw new Error('Turtle Club ended the session before saving the opponent update.');
    }
  } catch (error) {
