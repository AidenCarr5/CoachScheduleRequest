...
function statusPreviewShellUrls() {
  return [
    `${baseUrl}/CP/#Module=Webmaster;SelectedValue=Pages/1487/Status/`,
    `${baseUrl}/CP/#Module=Webmaster;SelectedValue=Pages/1487/STATUS/`,
    `${baseUrl}/CP/#Module=Webmaster;SelectedValue=%2FPages%2F1487%2FStatus%2F`,
    `${baseUrl}/CP/#Module=Webmaster;SelectedValue=%2FPages%2F1487%2FSTATUS%2F`
  ];
}
...
async function openStatusEditorFrame(page) {
  for (const shellUrl of statusPreviewShellUrls()) {
    await loginControlPanelShell(page, shellUrl);
    await page.waitForTimeout(2500);

    const frame = page.frames().find((candidate) => candidate.name() === 'rpMain');
    if (frame && (await frameContainsStatusEditor(frame) || await frameContainsStatusPagePreview(frame))) {
      return frame;
    }
  }
...
async function clickEditContent(page, previewFrame) {
...
async function applyStatusPageUpdate(page, previewFrame, update) {
  await clickEditContent(page, previewFrame);
  ...
  if (!editor) {
    ...
    throw new Error('The Turtle Club status page editor could not be found after clicking Edit Content.');
  }
