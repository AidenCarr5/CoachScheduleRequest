...
async function updateGameOpponent(request) {
  ...
  try {
    await loginSchedulingShell(page);
    const mainFrame = await openSchedulingMainFrame(page);
    await jumpToDayAtGlance(page, mainFrame, request.originalDate || request.date);
    const clicked = await clickMatchingDayEvent(page, mainFrame, {
      ...request,
      originalId: `tc-cp-${remoteId}`
    });
    if (!clicked) {
      saveDebugHtml('opponent-update-day-at-a-glance-not-found.html', await mainFrame.content().catch(() => ''));
      throw new Error('The game could not be found in Turtle Club Day At A Glance.');
    }
    await page.waitForTimeout(1500);
    const modalFrame = page.frames().find((candidate) => candidate.name() === 'rwForm');
    if (!modalFrame) {
    ...
    const submitted = await submitUpdateAndClose(modalFrame);
    if (!submitted) {
      saveDebugHtml('opponent-update-no-save-button.html', await modalFrame.content().catch(() => ''));
      throw new Error('The Turtle Club game editor did not show a save action.');
    }
    await page.waitForTimeout(2500);
  } catch (error) {
