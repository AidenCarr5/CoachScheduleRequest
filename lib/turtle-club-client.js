...
async function openStatusEditorFrame(page) {
  ...
  if (await frameContainsStatusEditor(mainFrame)) {
    return mainFrame;
  }
...
async function populateStatusEditorRow(frame, update) {
  ...
  const result = await frame.evaluate((payload) => {
    ...
    function containerControls(container) {
      return [...container.querySelectorAll('select, input, textarea, button, a')];
    }

    function closestContainer(element) {
      let current = element;
      while (current && current !== document.body) {
        const controlCount = containerControls(current).length;
        if (controlCount >= 3) return current;
    ...
    function candidateContainers() {
      return [...document.querySelectorAll('tr, fieldset, .rgRow, .rgAltRow, .form-group, .row, li, .content-row, .status-row, .RadGrid tr')]
        .filter((container) => containerControls(container).length >= 2);
    }

    const aliases = [...(payload.aliases || []), payload.group, payload.diamond, payload.label]
      .map((value) => visibleText(value))
      .filter(Boolean);
    const targetGroup = visibleText(payload.group);
    const targetDiamond = visibleText(payload.diamond);
    const expectedStatus = visibleText(payload.status);

    let bestContainer = null;
    let bestScore = -1;

    for (const container of candidateContainers()) {
      const text = visibleText(container.innerText || container.textContent || '');
      if (!text) continue;

      let score = 0;
      for (const alias of aliases) {
        if (text.includes(alias)) score += 4;
      }
      if (targetDiamond && new RegExp(`(^|\\s)${targetDiamond}(\\s|$)`).test(text)) score += 3;
      if (targetGroup) {
        let ancestor = container;
        while (ancestor && ancestor !== document.body) {
          const ancestorText = visibleText(ancestor.innerText || ancestor.textContent || '');
          if (ancestorText.includes(targetGroup)) {
            score += 4;
            break;
          }
    ...
    if ((!bestContainer || bestScore < 4) && targetDiamond) {
      const labelCandidates = [...document.querySelectorAll('label, strong, span, td, th, div')];
      for (const candidate of labelCandidates) {
        const text = visibleText(candidate.textContent || '');
        if (!text) continue;
        if (!aliases.some((alias) => text.includes(alias))) continue;
        const container = closestContainer(candidate);
        if (container) {
          bestContainer = container;
          bestScore = 10;
          break;
    ...
    if (!bestContainer) {
      return { ok: false, error: `Could not find the editable row for ${payload.label}.` };
    }

    bestContainer.setAttribute('data-codex-status-target', payload.targetId);

    function labelsForInput(input) {
      const labels = [];
      const id = input.id;
      if (id) {
        labels.push(...[...document.querySelectorAll(`label[for="${id}"]`)].map((label) => label.textContent || ''));
      }
      let parent = input.parentElement;
      while (parent && parent !== bestContainer.parentElement && parent !== document.body) {
        const label = parent.querySelector('label');
        if (label) labels.push(label.textContent || '');
        parent = parent.parentElement;
      }
      return visibleText(labels.join(' '));
    }

    const selects = [...bestContainer.querySelectorAll('select')];
    const radioButtons = [...bestContainer.querySelectorAll('input[type="radio"]')];
    const statusSelect = selects.find((select) => {
      const options = [...select.options].map((option) => visibleText(option.textContent || option.value || ''));
      return options.includes('open') && options.includes('closed');
    });

    let statusSet = false;
    if (statusSelect) {
      const option = [...statusSelect.options].find((candidate) => visibleText(candidate.textContent || candidate.value || '') === expectedStatus);
      if (option) {
    ...
    if (!statusSet && radioButtons.length) {
      const radio = radioButtons.find((candidate) => {
        const text = visibleText(candidate.value || labelsForInput(candidate));
    ...
    const textInputs = [...bestContainer.querySelectorAll('input:not([type="radio"]):not([type="checkbox"]):not([type="hidden"]), textarea')];
    const unusedInputs = new Set(textInputs);
    const usedNames = [];

    function setField(matchers, value) {
      if (value === undefined || value === null) return false;
      const normalizedMatchers = matchers.map((matcher) => visibleText(matcher));
      for (const input of textInputs) {
        if (!unusedInputs.has(input)) continue;
        const descriptor = visibleText([
          labelsForInput(input),
          input.getAttribute('placeholder') || '',
          input.name || '',
          input.id || '',
          input.className || ''
        ].join(' '));
        if (!normalizedMatchers.some((matcher) => descriptor.includes(matcher))) continue;
        ...
        unusedInputs.delete(input);
        usedNames.push(descriptor);
        return true;
      }
      return false;
    }

    setField(['date'], payload.dateText);
    setField(['time'], payload.timeText);
    setField(['updated by', 'updatedby', 'initials', 'user'], payload.initials);
    setField(['comments', 'comment', 'notes', 'note'], payload.notes);

    if (!usedNames.some((name) => name.includes('date'))) {
      const input = [...unusedInputs].find((candidate) => candidate.type === 'date');
      if (input) {
        input.value = payload.isoDate;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        unusedInputs.delete(input);
      }
    }
    if (!usedNames.some((name) => name.includes('time'))) {
      const input = [...unusedInputs].find((candidate) => candidate.type === 'time');
      if (input) {
        input.value = payload.isoTime;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        unusedInputs.delete(input);
      }
    }
    if (!usedNames.some((name) => name.includes('updated'))) {
      const input = [...unusedInputs].find((candidate) => candidate.tagName === 'INPUT');
      if (input) {
        input.value = payload.initials;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        unusedInputs.delete(input);
      }
    }
    if (!usedNames.some((name) => name.includes('comment') || name.includes('note'))) {
      const input = [...unusedInputs].find((candidate) => candidate.tagName === 'TEXTAREA')
        || [...unusedInputs].find((candidate) => candidate.tagName === 'INPUT');
      if (input) {
        input.value = payload.notes;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        unusedInputs.delete(input);
      }
    ...
    return {
      ok: statusSet,
      error: statusSet ? '' : `Could not find an Open/Closed control for ${payload.label}.`,
      rowText: bestContainer.innerText || ''
    };
  }, {
    ...target,
    status: update.status,
    notes: String(update.notes || ''),
    initials: String(update.initials || ''),
    dateText: editorDateText(update.requestedAt),
    timeText: editorTimeText(update.requestedAt),
    isoDate: editorIsoDate(update.requestedAt),
    isoTime: editorIsoTime(update.requestedAt)
  });

  if (!result.ok) {
    saveDebugHtml(`status-editor-row-${target.id}.html`, await frame.content().catch(() => ''));
    throw new Error(result.error || `Could not update ${target.label} on Turtle Club.`);
  }

  return target;
}

async function saveStatusEditorRow(frame, targetId) {
  const scoped = frame.locator(`[data-codex-status-target="${targetId}"]`).first();
  const rowSaveClicked = await clickFirstVisible([
    scoped.locator('button:has-text("Save")'),
    scoped.locator('button:has-text("Update")'),
    scoped.locator('button:has-text("Submit")'),
    scoped.locator('a:has-text("Save")'),
    scoped.locator('a:has-text("Update")'),
    scoped.locator('input[value*="Save"]'),
    scoped.locator('input[value*="Update"]')
  ]);
  if (rowSaveClicked) {
  ...
  const pageSaveClicked = await clickFirstVisible([
    ...
    frame.locator('button:has-text("Publish")'),
    ...
    frame.locator('input[value*="Publish"]')
  ]);

  if (!pageSaveClicked) {
    saveDebugHtml(`status-editor-save-${targetId}.html`, await frame.content().catch(() => ''));
...
async function updateDiamondStatus(update) {
  ...
  try {
    ...
    await populateStatusEditorRow(frame, update);
    await saveStatusEditorRow(frame, target.id);
    saveDebugHtml(`status-editor-after-save-${target.id}.html`, await frame.content().catch(() => ''));
    return {
      ok: true,
      targetId: target.id,
      label: target.label,
