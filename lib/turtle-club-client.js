...
async function selectOfficialInModal(modalFrame, official) {
  const choices = [
    official.username,
    official.name
  ].map((item) => String(item || '').trim()).filter(Boolean);
  const comboIds = await modalFrame.evaluate(() => {
    return [...document.querySelectorAll('input[id$="_Input"]')]
      .map((input) => String(input.id || '').replace(/_Input$/, ''))
      .filter((id) => window.$find && window.$find(id));
  });
  for (const comboId of comboIds) {
    for (const choice of choices) {
      try {
        return await selectInitializedComboItem(modalFrame, comboId, choice);
      } catch (_) {
        // Try the next combo/choice.
      }
    }
  }
  throw new Error(`Could not find ${official.name || official.username} in the Turtle Club official selector.`);
}

async function submitOfficialModal(modalFrame) {
  const clicked = await modalFrame.evaluate(() => {
    const candidates = [
      'cC_lbInsertAndClose',
      'ctl00_cC_lbInsertAndClose',
      'cC_lbSaveAndClose',
      'ctl00_cC_lbSaveAndClose',
      'cC_lbUpdateAndClose',
      'ctl00_cC_lbUpdateAndClose',
  ...
  if (!clicked) {
    throw new Error('The Turtle Club official assignment modal did not show a save action.');
  }
}

async function openOfficialAddModal(page, mainFrame, game, position) {
  ...
  const opened = await mainFrame.evaluate(({ remoteId: targetId, positionId: targetPositionId, positionLabel }) => {
    const row = document.getElementById(`multischedule_${targetId}`);
    if (!row) return { ok: false, reason: `Could not find game row ${targetId}.` };
    row.scrollIntoView({ block: 'center' });
    const assignmentRow = row.nextElementSibling;
    const searchRoot = assignmentRow || row;
    const candidates = [...searchRoot.querySelectorAll('a, button, input[type="button"], input[type="submit"]')];
    const labelNeedle = /home|plate/i.test(positionLabel) ? /home|plate|4010/i : /base|4020/i;
    const addControl = candidates.find((element) => {
      const text = `${element.textContent || ''} ${element.value || ''} ${element.title || ''} ${element.getAttribute('onclick') || ''}`;
      return /add|new|official/i.test(text) && labelNeedle.test(text);
    }) || candidates.find((element) => /add|new|official/i.test(`${element.textContent || ''} ${element.value || ''} ${element.title || ''} ${element.getAttribute('onclick') || ''}`));
    if (addControl) {
      addControl.click();
      return { ok: true, method: 'clicked-add-control' };
    }
    const hiddenPosition = document.getElementById('cMain_cBot_hfPosition');
    if (hiddenPosition) hiddenPosition.value = targetPositionId;
    if (typeof ShowNewGameOfficialForm === 'function') {
      ShowNewGameOfficialForm();
      return { ok: true, method: 'ShowNewGameOfficialForm' };
    }
    return { ok: false, reason: 'No add official control was available for this game.' };
  }, { remoteId, positionId, positionLabel: position });
  if (!opened.ok) {
    throw new Error(opened.reason || 'Could not open Turtle Club official assignment form.');
  }
  return waitForOfficialModal(page);
}
...
async function assignGameOfficial({ game, official, position }) {
  ...
  try {
    ...
    const mainFrame = await openOfficialsDailyFrame(page, game.date);
    saveDebugHtml('official-assign-daily.html', await mainFrame.content().catch(() => ''));
    const modalFrame = await openOfficialAddModal(page, mainFrame, game, position);
    saveDebugHtml('official-assign-modal.html', await modalFrame.content().catch(() => ''));
    const selected = await selectOfficialInModal(modalFrame, official);
    ...
    await submitOfficialModal(modalFrame);
...
async function removeGameOfficial({ game, assignment }) {
  ...
  try {
    await loginOfficialsShell(page);
    const mainFrame = await openOfficialsDailyFrame(page, game.date);
    const removed = await mainFrame.evaluate(({ remoteId, name, position }) => {
      const row = document.getElementById(`multischedule_${remoteId}`);
      const assignmentRow = row && row.nextElementSibling;
      if (!assignmentRow) return { ok: false, reason: 'Assignment row was not found.' };
      const normalize = (value) => String(value || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
      const targetName = normalize(name);
      const targetPosition = normalize(position);
      const officialNodes = [...assignmentRow.querySelectorAll('.gameOfficial, span')];
      const officialNode = officialNodes.find((node) => {
        const text = normalize(node.textContent);
        return text.includes(targetName) && (!targetPosition || text.includes(targetPosition));
      });
      if (!officialNode) return { ok: false, reason: 'Official was not found on the Turtle Club assignment row.' };
      const container = officialNode.closest('td, div, span') || assignmentRow;
      const control = [...container.querySelectorAll('a, button, input[type="button"]')]
        .find((element) => /delete|remove/i.test(`${element.textContent || ''} ${element.value || ''} ${element.title || ''} ${element.getAttribute('onclick') || ''}`));
      if (!control) return { ok: false, reason: 'Remove official control was not found.' };
      control.click();
      return { ok: true };
    }, {
      remoteId: game.remoteId || game.cpGameId || '',
      name: assignment.name || assignment.username,
      position: assignment.position || ''
    });
    if (!removed.ok) throw new Error(removed.reason || 'Could not remove official from Turtle Club.');
    await page.waitForTimeout(3000);
    return { ok: true };
  } catch (error) {
