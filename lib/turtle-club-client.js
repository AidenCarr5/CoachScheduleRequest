...
function tournamentScheduleUrl(tournamentId = 3331) {
  return `${baseUrl}/CP/Live/Tournaments/${encodeURIComponent(tournamentId)}/Schedule/`;
}
...
async function openTournamentScheduleFrame(page, tournamentId = 3331) {
  let frame = page.frames().find((candidate) => candidate.name() === 'rpMain');
  if (!frame) {
    throw new Error('Could not find the Turtle Club tournament frame.');
  }
  await frame.goto(tournamentScheduleUrl(tournamentId), { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2500);
  frame = page.frames().find((candidate) => candidate.name() === 'rpMain');
  if (!frame) {
...
async function listTournamentScoreGames({ tournamentId = 3331 } = {}) {
  ...
  try {
    ...
    const frame = await openTournamentScheduleFrame(page, tournamentId);
    ...
    const games = await frame.evaluate(() => {
      ...
      function rowAction(row) {
        ...
        const preferred = controls.find((control) => /score|report|edit|result/i.test(clean(control.textContent || control.value || '') + ' ' + attrText(control)))
          || controls.find((control) => /GameID|Score|Report|ID=/i.test(attrText(control)));
        if (!preferred) return {};
        return {
          href: preferred.href || preferred.getAttribute('href') || '',
          onclick: preferred.getAttribute('onclick') || '',
          text: clean(preferred.textContent || preferred.value || ''),
          id: preferred.id || '',
          name: preferred.name || ''
        };
      }
      ...
      return rows.map((row, index) => {
        ...
        return {
          ...
          action,
          actionSource: [action.href, action.onclick, action.id, action.name].filter(Boolean).join(' ')
        };
      });
    });

    return games
      .filter((row) => isLikelyTournamentGameRow(row.text) && (row.actionSource || /unreported|report score/i.test(row.text)))
      .map((row) => {
        const id = tournamentGameIdFromText(`${row.actionSource} ${row.text}`) || crypto.createHash('sha1').update(row.text).digest('hex').slice(0, 12);
        const cells = row.cells || [];
        const team = cells.find((cell) => /vanwezel|titans|lasalle|athletics|select|u\d+/i.test(cell)) || cells[3] || '';
        const opponent = cells.find((cell, index) => index > 2 && cell !== team && /[A-Za-z]/.test(cell) && !/diamond|field|innings|score|unreported/i.test(cell)) || '';
        const venue = cells.find((cell) => /diamond|field|park|vollmer|turtle|villanova|river/i.test(cell)) || '';
        return {
          id,
          tournamentId,
          rowIndex: row.rowIndex,
          date: parseTournamentDate(row.text),
          time: parseTournamentTime(row.text),
          team,
          opponent,
          venue,
          status: /unreported|report score/i.test(row.text) ? 'unreported' : 'available',
          action: row.action,
          summary: row.text
        };
      });
...
async function submitTournamentScore({ tournamentId = 3331, game, homeScore, awayScore }) {
  ...
  try {
    ...
    const filled = await editor.evaluate(({ home, away }) => {
      function clean(value) {
        return String(value || '').replace(/\s+/g, ' ').trim();
      }
      ...
      const chosen = (candidates.length >= 2 ? candidates : inputs).slice(0, 2);
      if (chosen.length < 2) return { ok: false, count: chosen.length };
      chosen[0].input.value = String(home);
      chosen[0].input.dispatchEvent(new Event('input', { bubbles: true }));
      chosen[0].input.dispatchEvent(new Event('change', { bubbles: true }));
      chosen[1].input.value = String(away);
      chosen[1].input.dispatchEvent(new Event('input', { bubbles: true }));
      chosen[1].input.dispatchEvent(new Event('change', { bubbles: true }));
      return {
        ok: true,
        fields: chosen.map((item) => ({ id: item.id, name: item.name, label: item.label }))
      };
