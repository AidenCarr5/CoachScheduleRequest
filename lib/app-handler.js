...
const { sendTestDiamondStatusAlert, sendCoachRequestSubmittedEmail, sendCoachRequestDecisionEmail, sendOpponentChangeEmail, sendOpponentChangeFailureEmail, sendApprovedRequestSyncFailureEmail, sendAllocationApprovalRequestEmail, smtpConfigured } = require('./diamond-status-monitor');
...
function findConfiguredAdminUser(username) {
  ...
  return configuredAdminUsers.find((user) => user.username.toLowerCase() === clean) || null;
}
