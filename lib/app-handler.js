...
function buildAvailabilityBlocks(data) {
  ...
  return (data.availability || [])
    .flatMap((slot) => {
      const start = minutesFromDisplay(slot.start);
      const end = minutesFromDisplay(slot.end);
      if (!slot.date || !slot.diamond || end - start < blockMinutes) return [];
      const blocks = [];
      for (let blockStart = start; blockStart + blockMinutes <= end; blockStart += stepMinutes) {
        blocks.push({
          date: slot.date,
          day: new Date(`${slot.date}T12:00:00`).toLocaleDateString(undefined, {
            weekday: 'long',
            month: 'short',
            day: 'numeric',
            year: 'numeric'
          }),
          month: new Date(`${slot.date}T12:00:00`).toLocaleDateString(undefined, { month: 'long', year: 'numeric' }),
          diamond: slot.diamond,
          start: displayFromMinutes(blockStart),
          end: displayFromMinutes(blockStart + blockMinutes),
    ...
    .sort((a, b) => `${a.date} ${minutesFromDisplay(a.start)} ${a.diamond}`.localeCompare(`${b.date} ${minutesFromDisplay(b.start)} ${b.diamond}`));
}
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/availability') {
    ...
    sendJson(res, 200, {
      blocks: buildAvailabilityBlocks(data),
