...
function baseUmpireStatus(event, category, filledOverride = null) {
  ...
  const confirmedCount = externallyFilled
    ? required
  ...
  return {
    ...
    source: overrideFilled ? 'manual-filled' : (autoConfirmed ? 'age-rule' : (status.source || '')),
    umpire1Confirmed: externallyFilled || Boolean(status.umpire1Confirmed),
    umpire2Confirmed: required < 2 ? true : (externallyFilled || Boolean(status.umpire2Confirmed))
  };
}
