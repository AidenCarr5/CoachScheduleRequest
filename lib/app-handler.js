...
function buildAvailabilityBlocks(data) {
  const dates = [...new Set((data.availability || []).map((slot) => slot.date).filter(Boolean))].sort();
  ...
  const days = dates.map((date) => {
    ...
    const slotStarts = isWeekend ? [480, 600, 720, 840, 960, 1080] : [1080];
    const slots = slotStarts.map((start) => ({
      start: displayFromMinutes(start),
      end: displayFromMinutes(start + 120),
      startMinutes: start,
      endMinutes: start + 120
    }));
    const diamondRows = diamonds.map((diamond) => {
      return {
        diamond,
        slots: slots.map((slot) => {
          const openSlot = (data.availability || []).find((available) => {
            return available.date === date
              && available.diamond === diamond
              && minutesFromDisplay(available.start) <= slot.startMinutes
              && minutesFromDisplay(available.end) >= slot.endMinutes;
          });
          const conflict = (data.conflictEvents || data.schedule || []).find((event) => {
            if (event.date !== date || event.diamond !== diamond) return false;
            ...
            return slot.startMinutes < eventEnd && slot.endMinutes > eventStart;
          });
          const available = Boolean(openSlot && !conflict);
          ...
          return {
            ...
            conflict: conflict ? `${conflict.team} ${conflict.opponent}` : '',
            sourceStart: openSlot ? openSlot.start : '',
            sourceEnd: openSlot ? openSlot.end : ''
          };
        })
      };
    });
