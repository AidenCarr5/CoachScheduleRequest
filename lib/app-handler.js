...
const { loadData, refreshData, refreshDateData, dataVersion } = require('./data-store');
const { applyApprovedRequest, updateGameOpponent, updateDiamondStatus } = require('./turtle-club-client');
const { sendTestDiamondStatusAlert, sendCoachRequestSubmittedEmail, sendCoachRequestDecisionEmail, sendOpponentChangeEmail, smtpConfigured } = require('./diamond-status-monitor');
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/coach/events/opponent') {
    ...
    if (session.role !== 'coach') {
      sendJson(res, 403, { error: 'Only coach accounts can directly change game opponents.' });
      return;
    }
    const payload = await readBody(req);
    ...
    let updateResult;
    try {
      updateResult = await updateGameOpponent(change);
    } catch (error) {
      sendJson(res, 502, {
        error: 'Turtle Club opponent update failed',
        details: error.message
      });
      return;
    }
    ...
    if (smtpConfigured()) {
      try {
        emailSent = await sendOpponentChangeEmail({
          ...change,
          remoteId: updateResult.remoteId
        });
      } catch (error) {
        emailError = error.message || 'Unknown email error';
        console.error(`Opponent change email failed for ${change.team} (${event.id}):`, emailError);
      }
    } else {
