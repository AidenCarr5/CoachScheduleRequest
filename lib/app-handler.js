...
function umpireRequiredSlots(event, category) {
  if (category === 'House League') return 1;
  return 2;
}
...
function buildUmpireGame(event, category, claims) {
...
async function umpirePortalGames() {
  ...
  const claims = Array.isArray(readUmpireStore().claims) ? readUmpireStore().claims : [];
  const seen = new Set();
  const rawGames = [];
  ...
  function addEvents(events, category) {
    (events || []).forEach((event) => {
      ...
      rawGames.push(buildUmpireGame(copy, category, claims));
    });
  }
  ...
  ].filter((event) => isHouseLeagueTeam(event.team)), 'House League');
