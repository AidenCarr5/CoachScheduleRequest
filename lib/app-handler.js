...
function updateCoachAccountPassword(username, password) {
  const normalized = String(username || '').toLowerCase();
  const store = readCoachAccountStore();
...
function filterDataForSession(data, session) {
  if (!session || session.role === 'admin' || session.role === 'status_editor') return data;
  return {
    ...data,
    teams: [session.team],
...
function requestVisibleToSession(request, session) {
  ...
  if (session.role === 'admin' || session.role === 'status_editor') return true;
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/coach/profile/change-password') {
    ...
    const updatedAccount = updateCoachAccountPassword(account.username, newPassword);
    if (!updatedAccount) {
