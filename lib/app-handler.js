...
function filterDataForSession(data, session) {
  const filtered = filterHiddenDiamonds(data);
  if (!session || session.role === 'admin') return filtered;
  return {
    ...filtered,
    teams: [session.team],
    schedule: filtered.schedule.filter((event) => event.team === session.team)
  };
}

function isHiddenDiamond(diamond) {
  return /river\s+canard/i.test(String(diamond || ''));
}

function filterHiddenDiamonds(data) {
  return {
    ...data,
    availability: (data.availability || []).filter((slot) => !isHiddenDiamond(slot.diamond)),
    conflictEvents: (data.conflictEvents || []).filter((event) => !isHiddenDiamond(event.diamond))
  };
}
...
function buildAvailabilityBlocks(data) {
  const visibleData = filterHiddenDiamonds(data);
  const dates = [...new Set((visibleData.availability || []).map((slot) => slot.date).filter(Boolean))].sort();
  const diamonds = [...new Set((visibleData.availability || []).map((slot) => slot.diamond).filter(Boolean))].sort();
  ...
  const days = dates.map((date) => {
    ...
    const dayAvailability = (visibleData.availability || []).filter((slot) => slot.date === date);
    ...
    const diamondRows = diamonds.map((diamond) => {
      const openRanges = (visibleData.availability || [])
        .filter((slot) => slot.date === date && slot.diamond === diamond)
        .map((slot) => clippedRange(minutesFromDisplay(slot.start), minutesFromDisplay(slot.end), windowStart, windowEnd))
        .filter((range) => range.end > range.start);
      const conflicts = (visibleData.conflictEvents || visibleData.schedule || [])
