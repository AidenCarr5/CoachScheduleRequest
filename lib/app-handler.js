...
function buildAvailabilityBlocks(data) {
  ...
  const diamonds = [...new Set([
    ...(data.availability || []).map((slot) => slot.diamond).filter(Boolean),
    ...((data.conflictEvents || data.schedule || []).map((event) => event.diamond).filter((diamond) => diamond && isHomeVenue(diamond)))
  ])].sort();
  ...
  const days = dates.map((date) => {
    ...
    const dayConflicts = (data.conflictEvents || data.schedule || [])
      .filter((event) => event.date === date && diamondSet.has(event.diamond));
