...
const { applyApprovedRequest, updateGameOpponent, updateDiamondStatus, assignGameOfficial, removeGameOfficial } = require('./turtle-club-client');
const { sendTestDiamondStatusAlert, sendCoachRequestSubmittedEmail, sendCoachRequestDecisionEmail, sendOpponentChangeEmail, sendOpponentChangeFailureEmail, sendAllocationApprovalRequestEmail, smtpConfigured } = require('./diamond-status-monitor');
...
function syncUmpireAccountsFromOfficials(officials) {
  ...
  officials.forEach((official) => {
    ...
    if (existing) {
      ...
      if (existing.name !== name || existing.source !== 'official-roster') {
        existing.name = name;
        existing.source = existing.source || 'official-roster';
...
function baseUmpireStatus(event, category) {
  const required = umpireRequiredSlots(event, category);
  const status = event.umpireStatus || {};
  const autoConfirmed = isHomeGameEvent(event) && gameAge(event.team) >= 14;
  const confirmedCount = autoConfirmed
    ? required
  ...
  return {
    ...
    source: autoConfirmed ? 'age-rule' : (status.source || ''),
    umpire1Confirmed: autoConfirmed || Boolean(status.umpire1Confirmed),
    umpire2Confirmed: required < 2 ? true : (autoConfirmed || Boolean(status.umpire2Confirmed))
  };
}
...
function officialEntryForEvent(official, accountByName) {
  if (!official) return null;
  const account = accountByName.get(normalizeOfficialName(official.name));
...
function officialsForEventByConfirmation(event, accountByName, confirmed) {
  ...
  return officials
    ...
    .map((official) => officialEntryForEvent(official, accountByName))
    .filter(Boolean);
}

function assignedOfficialsForEvent(event, accountByName) {
  return officialsForEventByConfirmation(event, accountByName, true);
}

function pendingOfficialsForEvent(event, accountByName) {
  return officialsForEventByConfirmation(event, accountByName, false)
    .filter((official) => !['denied', 'rejected', 'declined'].includes(official.status));
}

function rejectedOfficialsForEvent(event, accountByName) {
  return officialsForEventByConfirmation(event, accountByName, false)
    .filter((official) => ['denied', 'rejected', 'declined'].includes(official.status));
}

function buildUmpireGame(event, category, claims, assignments = [], accountMaps = { byName: new Map(), byUsername: new Map() }) {
  const status = baseUmpireStatus(event, category);
  const gameClaims = claims.filter((claim) => claim.gameId === event.__umpireId).map(sanitizeClaim);
  const remoteAssignedOfficials = assignedOfficialsForEvent(event, accountMaps.byName);
  ...
  const pendingOfficials = pendingOfficialsForEvent(event, accountMaps.byName);
  const rejectedOfficials = rejectedOfficialsForEvent(event, accountMaps.byName);
  ...
  return {
    ...
    filled: confirmedCount >= status.required,
...
async function umpirePortalGames() {
  ...
  const rawGames = [...gamesByKey.entries()].map(([key, item]) => {
    ...
    return buildUmpireGame(copy, item.category, claims, assignments, accountMaps);
  });
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/coach/login') {
    ...
    if (username.toLowerCase() === adminUsername.toLowerCase() && password === adminPassword) {
      const adminToken = createSessionToken({ role: 'admin', username: adminUsername, initials: 'AC' });
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      ...
      res.end(JSON.stringify({
        ok: true,
        user: { role: 'admin', username: adminUsername, team: '' },
  ...
  if (req.method === 'POST' && pathname === '/api/umpire/login') {
    ...
    if (username.toLowerCase() === String(adminUsername || '').toLowerCase() && password === adminPassword) {
      const token = createSessionToken({ role: 'admin', username: adminUsername, initials: 'AC' });
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Set-Cookie', [
  ...
  if (req.method === 'POST' && umpireAssignMatch) {
    ...
    if (game.filled) {
      sendJson(res, 409, { error: 'This game already has the required number of umpires.' });
      return;
    }
    const account = findUmpireAccount(username);
    if (!account) {
  ...
  if (req.method === 'POST' && umpireMoveClaimMatch) {
    ...
    if (targetGame.filled) {
      sendJson(res, 409, { error: 'Move availability only to a game that still needs an umpire.' });
      return;
    }
    const store = readUmpireStore();
    const claims = Array.isArray(store.claims) ? store.claims : [];
  ...
  if (req.method === 'GET' && pathname === '/api/admin/coach-accounts') {
    const session = readCoachSession(req);
    const canRevealPasswords = canMutateAdminPortalSession(session);
    const data = await loadData();
  ...
  if (req.method === 'POST' && approvalMatch) {
    ...
    if (action === 'approve') {
      ...
      let applyResult;
      try {
        if (isTournamentCancellationRequest(existing)) {
          applyResult = { outcome: 'cancelled-internal' };
          refreshedData = await loadData();
        } else {
    ...
    sendJson(res, 200, {
      ...
      emailSent,
      emailError,
      appliedToTurtleClub: !isManualApprove && !isTournamentCancellationRequest(existing),
      verified: existing.__verification ? existing.__verification.ok : true,
      verificationDetails: isManualApprove
        ? 'Manual approval only: Turtle Club was not updated by the scheduler.'
        : existing.__verification ? existing.__verification.details : 'No verification was required.'
    });
    return;
  }
