...
function buildAvailabilityBlocks(data) {
  ...
  const days = dates.map((date) => {
    ...
    const diamondRows = diamonds.map((diamond) => {
      ...
      const conflicts = (data.conflictEvents || data.schedule || [])
        ...
        .map((event) => {
          ...
          return {
            ...
            allHomeDiamonds: isAllHomeDiamondsEvent(event)
          };
        })
        .filter((range) => range.end > range.start);
      const fullRowConflict = conflicts.find((range) => {
        return range.allHomeDiamonds && range.start <= windowStart && range.end >= windowEnd;
      });
      if (fullRowConflict) {
        bookedCount += 1;
        return {
          diamond,
          segments: [{
            start: displayFromMinutes(fullRowConflict.start),
            end: displayFromMinutes(fullRowConflict.end),
            startMinutes: fullRowConflict.start,
            endMinutes: fullRowConflict.end,
            width: 100,
            status: 'booked',
            label: 'Booked / unavailable',
            conflict: fullRowConflict.label
          }]
        };
      }
      ...
      displayConflicts.forEach((range) => boundaries.push(range.start, range.end));
      ...
      for (let index = 0; index < sortedBoundaries.length - 1; index += 1) {
        ...
        const conflict = displayConflicts.find((range) => rangesOverlap(start, end, range.start, range.end));
