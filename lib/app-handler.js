...
async function refreshTournamentScoreCache(reason = 'scheduled') {
  ...
  tournamentScoreCacheRefreshPromise = (async () => {
    ...
    try {
      ...
      const reportedGames = await listReportedTournamentScoreGames({ tournamentId: tournamentScoresTournamentId });
      const bracket = await getTournamentBracketData({ tournamentId: tournamentScoresTournamentId });
      const cache = {
        tournamentId: tournamentScoresTournamentId,
        games,
        reportedGames,
      ...
      console.log(`[tournament-scores] refresh complete (${games.length} open, ${reportedGames.length} reported)`);
      return cache;
    } catch (error) {
      const previous = readTournamentScoreCache();
      previous.error = error.message || String(error);
      writeTournamentScoreCache(previous);
      console.error(`[tournament-scores] refresh failed: ${previous.error}`);
      throw error;
    } finally {
...
async function tournamentScoreCacheForRequest(req, reason) {
  if (shouldForceTournamentScoreRefresh(req)) {
    return refreshTournamentScoreCache(reason);
  }
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/tournament-scores/games') {
    ...
    try {
      const cache = await tournamentScoreCacheForRequest(req, 'manual score-games');
      sendJson(res, 200, {
        ok: true,
        tournamentId: tournamentScoresTournamentId,
        games: cache.games,
  ...
  if (req.method === 'GET' && pathname === '/api/tournament-scores/reported') {
    ...
    try {
      const cache = await tournamentScoreCacheForRequest(req, 'manual reported-scores');
      sendJson(res, 200, {
        ok: true,
        tournamentId: tournamentScoresTournamentId,
  ...
  if (req.method === 'GET' && pathname === '/api/tournament-scores/bracket') {
    ...
    try {
      const cache = await tournamentScoreCacheForRequest(req, 'manual tournament-bracket');
      sendJson(res, 200, {
        ok: true,
