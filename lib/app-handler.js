...
const { applyApprovedRequest } = require('./turtle-club-client');
...
function syncCoachAccounts(data) {
  ...
  const synced = generated.map((account) => {
    ...
    return {
      ...
      password: existing && existing.password ? String(existing.password) : coachPassword,
...
function eventMatchesRequestShape(event, request) {
  ...
  if (normalizeCompare(event.diamond) !== normalizeCompare(request.diamond)) return false;
  ...
  if (requestType.includes('away') && !eventKind.includes('away')) return false;
  if (requestType.includes('home') && !eventKind.includes('home')) return false;
  if (requestType.includes('game') && !eventKind.includes('game')) return false;
  ...
  if (opponent && opponent !== 'practice') {
    ...
    if (eventOpponent !== opponent && !eventOpponent.includes(opponent) && !opponent.includes(eventOpponent)) {
      return false;
    }
  }
...
function findVerifiedCreatedEvent(data, request) {
  const items = [...(data.conflictEvents || []), ...(data.schedule || [])];
  return items.find((event) => eventMatchesRequestShape(event, request)) || null;
}

function verifyApprovedRequestApplied(request, data) {
  const action = String(request.action || '');
  const createdEvent = findVerifiedCreatedEvent(data, request);
...
function validateRequestAgainstLiveData(request, data, queuedRequests, original) {
  ...
  (queuedRequests || [])
    ...
    .forEach((item) => {
      if (item.originalDate !== date || item.originalDiamond !== diamond || !item.originalId) return;
  ...
  const conflict = (data.conflictEvents || data.schedule || []).find((item) => {
    ...
    if (item.date !== date || item.diamond !== diamond) return false;
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && approvalMatch) {
    ...
    if (action === 'approve') {
      let refreshedData;
      try {
        await applyApprovedRequest(existing);
        refreshedData = await refreshData();
        const verification = verifyApprovedRequestApplied(existing, refreshedData);
