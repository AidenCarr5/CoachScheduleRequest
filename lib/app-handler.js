...
function findConfiguredAdminUser(username) {
  ...
  const user = configuredAdminUsers.find((item) => item.username.toLowerCase() === clean);
  if (!user) return null;
...
function adminLoginForCredentials(username, password, options = {}) {
  ...
  const candidates = [primary, ...configuredAdminUsers.map((user) => ({
    ...user,
    accessLabel: user.accessLabel || 'Can approve and reject coach schedule requests for this site.'
  }))];
...
function canSwitchAdminSitesSession(session) {
  return canAccessAdminPortalSession(session) && session.canSwitchSites !== false;
}
...
function publishedTeamsFromData(data, year) {
  ...
  const teams = [];
  [...(data && data.schedule || []), ...(data && data.conflictEvents || [])].forEach((event) => {
    if (String(event && event.date || '').slice(0, 4) === String(wantedYear) && event.team) {
      teams.push(event.team);
    }
  });
  return uniquePublishedTeams(teams);
}
...
function publicScheduleYearLinks(html, year) {
  const wantedYear = String(year);
  const urls = [];
  for (const match of String(html || '').matchAll(/href="([^"]*(?:Month=\d+&amp;Year=|Month=\d+&Year=)[^"]*)"/gi)) {
    const href = match[1].replace(/&amp;/g, '&');
    if (!new RegExp(`(?:\\?|&)Year=${wantedYear}(?:&|$)`).test(href)) continue;
    urls.push(new URL(href, turtleClubBaseUrl).toString());
  }
  return [...new Set(urls)];
}

async function discoverPublishedSeasonCoaches(year, data) {
  const wantedYear = Number(year) || new Date().getFullYear();
  const fromData = publishedTeamsFromData(data, wantedYear);
  const currentDataYear = Number(data && data.seasonYear);
  const searchedUrls = [];
  let source = fromData.length ? 'current bundled data' : '';
  let warning = '';

  if (fromData.length && currentDataYear === wantedYear) {
    return {
      year: wantedYear,
      source,
      searchedUrls,
  ...
  try {
    ...
    const yearLinks = publicScheduleYearLinks(indexHtml, wantedYear);
    const teamNames = [];

    if (!yearLinks.length && currentDataYear === wantedYear) {
      teamNames.push(...extractPublishedTeamsFromText(scheduleContentText(indexHtml)));
      searchedUrls.push(publicScheduleUrl);
    }

    for (const url of yearLinks) {
    ...
    return {
      ...
      teams: fromData,
      coaches: fromData.map((team) => ({ team, email: '', program: publishedCoachProgram() })),
      warning: fromData.length ? error.message : `Could not search Turtle Club for ${wantedYear}: ${error.message}`
    };
  }
}
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/admin/privileges') {
    ...
    const configurable = configuredAdminUsers.map((user) => findConfiguredAdminUser(user.username) || user);
    sendJson(res, 200, {
      admins: [
        {
          username: adminUsername,
  ...
  if (req.method === 'POST' && pathname === '/api/admin/privileges') {
    ...
    const payload = await readBody(req);
    const configurable = new Set(configuredAdminUsers.map((user) => user.username.toLowerCase()));
  ...
  if (req.method === 'POST' && pathname === '/api/admin/season-planner/create') {
    ...
    const payload = await readBody(req);
    const season = seasonPlanner.ensureSeason(payload.year, payload.label);
    sendJson(res, 200, {
      ok: true,
  ...
  if (req.method === 'POST' && pathname === '/api/admin/season-planner/discover-coaches') {
    ...
    const payload = await readBody(req);
    const data = await loadData();
    const discovered = await discoverPublishedSeasonCoaches(payload.year, data);
    sendJson(res, 200, {
      ok: true,
      ...discovered
    });
  ...
  if (req.method === 'GET' && pathname === '/api/admin/logins') {
    ...
    const configuredLogins = configuredAdminUsers.map((user) => ({
      label: user.accessLabel ? 'Site Admin' : 'Athletics Admin',
      username: user.username,
