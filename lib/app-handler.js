...
const { applyApprovedRequest, updateGameOpponent, updateDiamondStatus } = require('./turtle-club-client');
...
function umpireRequiredSlots(event, category) {
  if (String(category || '').startsWith('House League')) return 1;
  return 2;
}
...
function buildUmpireGame(event, category, claims, accountByName = new Map()) {
  ...
  const assignedOfficials = assignedOfficialsForEvent(event, accountByName);
  const pendingOfficials = pendingOfficialsForEvent(event, accountByName);
  return {
    id: event.__umpireId,
    gameNumber: event.gameNumber || event.gameNo || event.cpGameId || '',
    category,
    date: event.date || '',
    month: event.month || '',
    time: event.time || '',
    ...
    confirmedUmpires: status.confirmedCount,
    filled: status.filled,
    umpire1Confirmed: status.umpire1Confirmed,
    umpire2Confirmed: status.umpire2Confirmed,
    assignedOfficials,
    pendingOfficials,
...
async function umpirePortalGames() {
  ...
  function addEvents(events, categoryOrResolver) {
    (events || []).forEach((event) => {
      ...
      seen.add(key);
      const copy = { ...event, __umpireId: `ump-${crypto.createHash('sha1').update(key).digest('hex').slice(0, 16)}` };
      rawGames.push(buildUmpireGame(copy, category, claims, accountMaps.byName));
    });
  }
