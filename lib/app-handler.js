...
const { applyApprovedRequest, updateGameOpponent, updateDiamondStatus, assignGameOfficial, removeGameOfficial, listTournamentScoreGames, listReportedTournamentScoreGames, submitTournamentScore, getTournamentBracketData } = require('./turtle-club-client');
const { sendTestDiamondStatusAlert, sendCoachRequestSubmittedEmail, sendCoachRequestDecisionEmail, sendOpponentChangeEmail, sendOpponentChangeFailureEmail, sendApprovedRequestSyncFailureEmail, sendTournamentScoreFailureEmail, sendAllocationApprovalRequestEmail, sendSeasonUploadInviteEmail, smtpConfigured } = require('./diamond-status-monitor');
...
const tournamentScoreCacheFile = path.join(
  storageDir,
  `tournament-score-cache-${String(teamLabel || 'site').toLowerCase().replace(/[^a-z0-9]+/g, '-')}.json`
);
const tournamentScoreCacheTtlMs = 60 * 60 * 1000;
let tournamentScoreCacheRefreshPromise = null;
...
if (!fs.existsSync(tournamentScoreCacheFile)) {
  fs.writeFileSync(tournamentScoreCacheFile, JSON.stringify({
...
function opponentMatchesKnownPortalTeam(opponent, data) {
  const normalizedOpponent = normalizeCompare(opponent);
  if (!normalizedOpponent) return false;
  const teams = Array.isArray(data && data.teams) ? data.teams : [];
  return teams.some((team) => {
    const normalizedTeam = normalizeCompare(team);
    if (!normalizedTeam) return false;
    const withTeamLabel = normalizeCompare(`${teamLabel} ${team}`);
    const withBrandName = normalizeCompare(`${brandName} ${team}`);
    return normalizedOpponent === normalizedTeam
      || normalizedOpponent === withTeamLabel
      || normalizedOpponent === withBrandName
      || normalizedOpponent.endsWith(` ${normalizedTeam}`);
  });
}

function normalizeLocalGameRequest(request, data) {
  if (!request) return;
  const typeText = `${request.newType || ''} ${request.action || ''}`;
  if (!/\bgame\b/i.test(typeText)) return;
  if (/\baway\b/i.test(typeText) || /\bpractice\b/i.test(typeText) || /\btournament\b/i.test(typeText)) return;
  if (!opponentMatchesKnownPortalTeam(request.opponent, data)) return;

  request.newType = 'Local Game';
  if (request.action) {
    if (/\bHome Game\b/i.test(request.action)) {
      request.action = request.action.replace(/\bHome Game\b/i, 'Local Game');
    } else if (!/\bLocal Game\b/i.test(request.action)) {
      request.action = request.action.replace(/\bGame\b/i, 'Local Game');
    }
  }
}
...
function requestOrEventIsLocalGame(request, event) {
  return /\blocal\s+game\b/i.test(`${request && request.originalType || ''} ${request && request.newType || ''} ${request && request.action || ''} ${event && event.type || ''} ${event && event.eventKind || ''}`);
}
...
function eventMatchesRequestShape(event, request) {
  ...
  if (requestType.includes('home') && !(eventKind.includes('home') || eventKind.includes('local'))) return false;
  if (requestType.includes('local') && requestType.includes('game') && !(eventKind.includes('local') || eventKind.includes('home') || eventKind.includes('game'))) return false;
  ...
  if (opponent && opponent !== 'practice') {
    ...
    if (requestOrEventIsLocalGame(request, event) && (!eventOpponent || !opponent)) {
      return true;
    }
...
function readTournamentScoreCache() {
  try {
    const cache = JSON.parse(fs.readFileSync(tournamentScoreCacheFile, 'utf8'));
    return {
      tournamentId: Number(cache.tournamentId || tournamentScoresTournamentId),
      games: Array.isArray(cache.games) ? cache.games : [],
      reportedGames: Array.isArray(cache.reportedGames) ? cache.reportedGames : [],
      bracket: cache.bracket || null,
...
function writeTournamentScoreCache(cache) {
  fs.writeFileSync(tournamentScoreCacheFile, JSON.stringify(cache, null, 2));
}

function tournamentScoreCacheAgeMs(cache) {
  const timestamp = Date.parse(cache && cache.refreshedAt || '');
  if (!Number.isFinite(timestamp)) return Infinity;
  return Math.max(0, Date.now() - timestamp);
}

function tournamentScoreCacheIsStale(cache) {
  return tournamentScoreCacheAgeMs(cache) >= tournamentScoreCacheTtlMs;
}

async function refreshTournamentScoreCache(reason = 'scheduled', section = 'all') {
  if (tournamentScoreCacheRefreshPromise) return tournamentScoreCacheRefreshPromise;
  tournamentScoreCacheRefreshPromise = (async () => {
    try {
      console.log(`[tournament-scores] ${reason} refresh started`);
      const cache = readTournamentScoreCache();
      const errors = [];
      let changed = false;
      const include = (name) => section === 'all' || section === name;

      if (include('games')) {
        try {
          cache.games = await listTournamentScoreGames({ tournamentId: tournamentScoresTournamentId });
          changed = true;
        } catch (error) {
          errors.push(`open games: ${error.message || error}`);
        }
      }

      if (include('reported')) {
        try {
          cache.reportedGames = await listReportedTournamentScoreGames({ tournamentId: tournamentScoresTournamentId });
          changed = true;
        } catch (error) {
          errors.push(`reported scores: ${error.message || error}`);
        }
      }

      if (include('bracket')) {
        try {
          cache.bracket = await getTournamentBracketData({ tournamentId: tournamentScoresTournamentId });
          changed = true;
        } catch (error) {
          errors.push(`bracket: ${error.message || error}`);
        }
      }

      cache.tournamentId = tournamentScoresTournamentId;
      cache.refreshedAt = changed ? new Date().toISOString() : cache.refreshedAt;
      cache.error = errors.join(' | ');
      writeTournamentScoreCache(cache);

      if (errors.length) {
        console.error(`[tournament-scores] refresh partially failed: ${cache.error}`);
        if (!changed) throw new Error(cache.error);
      }
      console.log(`[tournament-scores] refresh complete (${cache.games.length} open, ${cache.reportedGames.length} reported)`);
      return cache;
    } finally {
      tournamentScoreCacheRefreshPromise = null;
    }
  })();
  return tournamentScoreCacheRefreshPromise;
}

function ensureTournamentScoreCacheFresh() {
  const cache = readTournamentScoreCache();
  if (tournamentScoreCacheIsStale(cache)) {
    refreshTournamentScoreCache(cache.refreshedAt ? 'hourly stale-cache' : 'startup empty-cache').catch(() => {});
  }
  return cache;
}

function shouldForceTournamentScoreRefresh(req) {
  try {
    const requestUrl = new URL(req.url || '', 'http://local');
    return ['1', 'true', 'yes'].includes(String(requestUrl.searchParams.get('refresh') || '').toLowerCase());
  } catch (_) {
    return false;
  }
}

async function tournamentScoreCacheForRequest(req, reason, section = 'all') {
  if (shouldForceTournamentScoreRefresh(req)) {
    return refreshTournamentScoreCache(reason, section);
  }
  return ensureTournamentScoreCacheFresh();
}

setTimeout(() => {
  ensureTournamentScoreCacheFresh();
}, 5000);

setInterval(() => {
  refreshTournamentScoreCache('hourly automatic').catch(() => {});
}, tournamentScoreCacheTtlMs);
...
function queueTournamentScoreSync(job) {
  setImmediate(() => {
    syncTournamentScoreToTurtleClub(job).catch((error) => {
      console.error(`Tournament score background sync crashed for ${job.id}:`, error.message || error);
    });
  });
}
...
function tournamentScoreGameMatches(left, right) {
  if (!left || !right) return false;
  const leftId = String(left.id || '').trim();
  const rightId = String(right.id || '').trim();
  if (leftId && rightId && leftId === rightId) return true;
  const leftGameNumber = String(left.gameNumber || '').trim().toLowerCase();
  const rightGameNumber = String(right.gameNumber || '').trim().toLowerCase();
  const leftDivision = String(left.division || '').trim().toLowerCase();
  const rightDivision = String(right.division || '').trim().toLowerCase();
  return Boolean(leftGameNumber && rightGameNumber && leftGameNumber === rightGameNumber
    && (!leftDivision || !rightDivision || leftDivision === rightDivision));
}

function tournamentScoreIsVisible(game, job) {
  const visitorScore = normalizeScoreValue(game.visitorScore ?? game.awayScore);
  const homeScore = normalizeScoreValue(game.homeScore);
  return visitorScore === normalizeScoreValue(job.awayScore ?? job.visitorScore)
    && homeScore === normalizeScoreValue(job.homeScore);
}

async function verifyTournamentScoreJob(job) {
  const games = await listReportedTournamentScoreGames({ tournamentId: job.tournamentId });
  const match = games.find((game) => tournamentScoreGameMatches(game, job.game));
  return Boolean(match && tournamentScoreIsVisible(match, job));
}

async function syncTournamentScoreToTurtleClub(job) {
  const maxAttempts = 3;
  let lastError = '';
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      await submitTournamentScore({
        ...
        game: job.game,
        homeScore: job.homeScore,
        awayScore: job.awayScore,
        visitorDefensiveInnings: job.visitorDefensiveInnings,
        homeDefensiveInnings: job.homeDefensiveInnings,
        visitorDefensiveOuts: job.visitorDefensiveOuts,
        homeDefensiveOuts: job.homeDefensiveOuts
      });
      job.status = 'synced';
      job.syncedAt = new Date().toISOString();
      refreshTournamentScoreCache('score sync follow-up').catch(() => {});
      return;
    } catch (error) {
      lastError = error.message || String(error);
      job.error = lastError;
      try {
        if (await verifyTournamentScoreJob(job)) {
          job.status = 'synced';
          job.syncedAt = new Date().toISOString();
          job.error = '';
          refreshTournamentScoreCache('score verification follow-up').catch(() => {});
          return;
        }
      } catch (verifyError) {
        job.error = `${lastError} | Verification failed: ${verifyError.message || verifyError}`;
      }
    }
  }
  ...
  if (smtpConfigured()) {
    try {
      await sendTournamentScoreFailureEmail(job);
    } catch (error) {
      console.error(`Tournament score failure email failed for ${job.id}:`, error.message || error);
    }
  } else {
    console.error(`Tournament score ${job.id} failed after ${job.attempts || maxAttempts} attempts:`, job.error || lastError);
  }
}
...
function queuedScheduleAdjustments(data, queuedRequests, request, date, normalizedDiamond, ignoredId, original) {
  ...
  (queuedRequests || [])
    ...
    .forEach((item) => {
      ...
      const replacingThisQueuedRequest = original && queuedRequestMatchesEvent(item, original);
      if ((action.startsWith('Cancel ') || action.startsWith('Replace ')) && item.originalId) {
        if (!(ignoredId && item.originalId === ignoredId) && !replacingThisQueuedRequest) {
...
function queuedRequestMatchesEvent(request, event) {
  ...
  if (normalizeCompare(request.team) !== normalizeCompare(event.team)) return false;
  ...
  if (!requestEventKindsMatch(request.newType || request.action, event.eventKind || event.type)) return false;
  const requestOpponent = normalizeRequestOpponent(request.opponent);
  const eventOpponent = normalizeRequestOpponent(event.opponent);
  return Boolean(requestOpponent && eventOpponent
    && (requestOpponent === eventOpponent || requestOpponent.includes(eventOpponent) || eventOpponent.includes(requestOpponent)));
}

function normalizeRequestOpponent(value) {
...
function requestEventKindsMatch(left, right) {
  const a = normalizeGameKind(left);
  const b = normalizeGameKind(right);
  if (a === b) return true;
  const homeLike = new Set(['home game', 'local game']);
  return homeLike.has(a) && homeLike.has(b);
}
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/tournament-scores/games') {
    ...
    try {
      const cache = await tournamentScoreCacheForRequest(req, 'manual score-games', 'games');
      sendJson(res, 200, {
        ok: true,
        tournamentId: tournamentScoresTournamentId,
        games: cache.games,
        refreshedAt: cache.refreshedAt,
        stale: tournamentScoreCacheIsStale(cache),
  ...
  if (req.method === 'GET' && pathname === '/api/tournament-scores/reported') {
    const session = readCoachSession(req);
    if (!canAccessTournamentScoresSession(session)) {
      sendJson(res, 403, { error: 'Tournament score access is limited to vanWezel and admin accounts.' });
      return;
    ...
    try {
      const cache = await tournamentScoreCacheForRequest(req, 'manual reported-scores', 'reported');
      sendJson(res, 200, {
        ok: true,
        tournamentId: tournamentScoresTournamentId,
        games: cache.reportedGames,
        refreshedAt: cache.refreshedAt,
        stale: tournamentScoreCacheIsStale(cache),
        cacheError: cache.error || ''
      });
      ...
      sendJson(res, 502, {
        error: 'Reported tournament scores could not be loaded from Turtle Club.',
        details: error.message || String(error)
      });
    }
    return;
  ...
  if (req.method === 'GET' && pathname === '/api/tournament-scores/bracket') {
    const session = readCoachSession(req);
    if (!canAccessTournamentScoresSession(session)) {
      sendJson(res, 403, { error: 'Tournament score access is limited to vanWezel and admin accounts.' });
      return;
    ...
    try {
      const cache = await tournamentScoreCacheForRequest(req, 'manual tournament-bracket', 'bracket');
      sendJson(res, 200, {
        ok: true,
        bracket: cache.bracket,
        refreshedAt: cache.refreshedAt,
        stale: tournamentScoreCacheIsStale(cache),
        cacheError: cache.error || ''
      });
      ...
      sendJson(res, 502, {
        error: 'Tournament bracket could not be loaded from Turtle Club.',
        details: error.message || String(error)
      });
    }
    return;
  ...
  if (req.method === 'POST' && pathname === '/api/tournament-scores/report') {
    ...
    const payload = await readBody(req);
    const job = {
      id: `score_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`,
      status: 'queued',
      tournamentId: tournamentScoresTournamentId,
      game: payload.game,
      homeScore: payload.homeScore,
      awayScore: payload.awayScore ?? payload.visitorScore,
      visitorScore: payload.visitorScore ?? payload.awayScore,
      visitorDefensiveInnings: payload.visitorDefensiveInnings,
      homeDefensiveInnings: payload.homeDefensiveInnings,
      visitorDefensiveOuts: payload.visitorDefensiveOuts,
      homeDefensiveOuts: payload.homeDefensiveOuts,
      requestedBy: session.username || session.team || session.type || 'tournament-score-user',
      queuedAt: new Date().toISOString(),
      attempts: 0
    };
    queueTournamentScoreSync(job);
    sendJson(res, 202, {
      ok: true,
      queued: true,
      jobId: job.id,
      submittedAt: job.queuedAt,
      message: 'Score update queued. Turtle Club sync is running in the background.'
    });
    return;
  }
  ...
  if (req.method === 'POST' && pathname === '/api/requests') {
    ...
    normalizeLocalGameRequest(payload, fullData);
