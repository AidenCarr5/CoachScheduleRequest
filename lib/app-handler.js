...
function umpirePathForSession(session) {
  return canAccessUmpirePortalSession(session) ? '/umpire-availability.html' : '';
}
...
function assignedOfficialsForEvent(event, accountByName) {
  const officials = Array.isArray(event && event.umpireStatus && event.umpireStatus.officials)
    ...
    : [];
  return officials
    .filter((official) => official && official.confirmed)
    .map((official) => {
      const account = accountByName.get(normalizeOfficialName(official.name));
      return {
        username: account ? account.username : '',
        name: official.name || (account && account.name) || '',
...
function allocationReviewForRequest(request) {
  ...
  return {
    ...
    status: 'pending',
    assignedSlot,
    assignedOwnerLabel: assignedSlot && assignedSlot.team ? assignedSlot.team : 'No listed Titans allotment for this diamond/time.',
    reason: assignedSlot
      ? `Outside ${request.team}'s allotted time. This slot is allotted to ${assignedSlot.team}.`
      : `Outside ${request.team}'s allotted time. No Titans allotment was found for this diamond/time.`
  };
}
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/umpire/session') {
    ...
    sendJson(res, 200, {
      ...
      user: canAccessUmpirePortalSession(session) ? {
        role: session.role,
        username: session.username || '',
  ...
  if (req.method === 'GET' && allocationApprovalMatch) {
    const tokenPayload = parseSignedToken(decodeURIComponent(allocationApprovalMatch[1] || ''));
    ...
    const request = await updateRequestStore(requestId, (item) => {
      ...
      if (decision === 'reject') {
        ...
        item.adminNote = 'Rejected during additional diamond allotment approval.';
  ...
  if (req.method === 'POST' && approvalMatch) {
    ...
    if (action === 'approve' && allocationStatus === 'pending') {
      sendJson(res, 409, {
    ...
    if (action === 'approve' && allocationStatus === 'rejected') {
      sendJson(res, 409, {
    ...
    sendJson(res, 200, {
      ...
      emailSent,
      emailError,
      appliedToTurtleClub: !isTournamentCancellationRequest(existing),
