...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/requests') {
    ...
    sendCoachRequestSubmittedEmail(stored).catch((error) => {
      console.error('Request submission email failed:', error.message);
    });
  ...
  if (req.method === 'DELETE' && coachDeleteMatch) {
    ...
    if (session.role !== 'admin' && request.team !== session.team) {
      sendJson(res, 403, { error: 'This coach cannot delete another team request' });
      return;
    }
  ...
  if (req.method === 'POST' && approvalMatch) {
    ...
    sendJson(res, 200, { request, emailSent });
    return;
  }
