...
function eventMatchesRequestShape(event, request) {
  ...
  if ((requestType.includes('away') || requestType.includes('home') || requestType.includes('game')) && !eventKind.includes('game')) return false;
  ...
  if (opponent && opponent !== 'practice') {
    ...
    if (!labelsLikelyMatch(eventOpponent, opponent)) {
      return false;
    }
  }
