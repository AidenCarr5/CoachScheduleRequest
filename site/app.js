(function () {
  ...
  const months = ['All months', ...new Set(data.schedule.map((event) => event.month))];
  const diamonds = [...new Set(data.availability.map((slot) => slot.diamond))].sort();
  const dateBounds = getDateBounds();
  let refreshTimer = 0;
  let currentDataVersion = data.scrapedAt || '';
